...
<?php 
if($row['share'] != 1){
	$imagepath=$base_url.'group_photo/';
...
<?php 
	list($width, $height) = getimagesize($base_url.'group_photo/'.$row['messages']);
	if($width > 600)
	{
	?>
    <img src="<?php echo $imagepath.$row['messages'];?>" class="stimage"/>
    <?php } 
	else if($width <= 600)
	{
	?>
...
<!--<input type="button" value="show smiley" id="<?php echo $row['messages_id'];?>" onclick="show(this.id)"  />!--><a herf="#!" style="cursor:pointer;" onclick="show(this.id)" id="<?php echo $row['messages_id'];?>"><img src="images/Glad.png"></a>
<!--code for smiley!-->
