...
ini_set('max_execution_time', 300); 	
	...
	$likeq=mysqli_query($con, "SELECT * FROM members WHERE member_id='$member_id'");
	$uname=mysqli_fetch_array($likeq);
	$likeusername=$uname["username"];
...
$('.translateButton<?php echo $_POST["lastid"];?>').click(function (event) {
        var ID = $(this).attr('id');
    ...
    $('.posttranslateButton<?php echo $_POST["lastid"];?>').click(function (event) {
        var ID = $(this).attr('id');
	...
	$('.replytranslateButton<?php echo $_POST["lastid"];?>').click(function (event) {
	    var ID = $(this).attr('id');
...
$query = "SELECT msg.content_id, msg.country_flag, msg.date_created, msg.messages_id, msg.member_id,m.active,msg.share_msg,
m.profImage, m.username, msg.type, msg.messages, msg.msg_album_id, u.upload_data_id,msg.share, msg.share_by, msg.photo_status,m.username,
v.location,v.location1,v.location2, v.thumburl, v.title, v.video_id, v.description,
 			v.msg_id, v.category, v.title_color,v.title_size,v.ads,a.ads_name,
				a.location as adslocation,a.location1 as adslocation1,a.location2 as adslocation2, a.click_url FROM message msg
...
ORDER BY msg.date_created DESC LIMIT 5";
//echo $query;
$count_home_query=mysqli_num_rows(mysqli_query($con, $query));
...
if($block_count==0 && $row['active']==0)
	...
	$country_code1 = str_replace('-', ' ', $row['country_flag']);
	$cfsql = mysqli_query($con, "select * from geo_country where country_title='".$country_code1."'") or die(mysqli_error($con));
...
<?php } } } }
 ...
 if($row['country_flag']==NULL && $row['country_id'] != 0){

//video country id fetch
	$count_c = explode(",",$row['country_id']);
	//echo count($count_c)."<br />";
	?> <?php echo $lang['Streamed Video in'];?><?php
	if(count($count_c)==1){
	...
	<span id="showCountOfCountries<?php echo $row['messages_id'];?>" onclick="hideShowOfCountryList(<?php echo $row['messages_id'];?>);"><a href="javascript:void(0)"><?php echo count($count_c).' Countries';?></a></span><?php
	$andCount=intval(count($count_c))-2;
	$lastCount=intval(count($count_c))-1;
	?>
	<span style="display:none;" id="showFullCountryList<?php echo $row['messages_id'];?>">
	<?php
...
if($r_count>0)
{
$row_post1 = mysqli_fetch_assoc($sql);


	echo "<p style='display: inline;]>". $row_post1['message']."</p>";
...
var text="<?php echo $row['messages'];?>";
...
//alert(src);
    ...
    var requestStr = "<?php echo $base_url;?>token.php";
    //sleep(1000);
    $.ajax({
        url: requestStr,
        type: "GET",
...
?></div>
<a href="<?php echo $base_url;?>albums.php?back_page=<?php echo $base_url.'i/'.$likeusername;?>&member_id=<?php echo $row['member_id']; ?>&album_id=<?php echo $row['msg_album_id']; ?>&image_id=<?php echo $row['upload_data_id'];?>" >
...
$row_comment = mysqli_fetch_assoc($sql);
	...
	//alert("inside");
var lan12="<?php echo $_SESSION['lang'];?>";
var id22="<?php echo $row1['comment_id'];?>";
var text2="<?php echo $row1['content'];?>";
//alert("inside1");
...
function translate_back_wall1222<?php echo $x;?>(idbd,g_token,src1,type1,lan)
	...
	{
	//alert(response);
	document.getElementById("tr_wall_co_back<?php echo $x;?>").innerHTML = response;
	}
});
	}
	
</script>
<div id="tr_wall_co_back<?php echo $x;?>"></div> 
<?php $x++; 
...
if($row1['type']==2) echo '<img src="'.$base_url.$row1["content"].'" >';
?>
...
<span class="mySpan_dot_class"> · </span>
 <a class="translateButton" href="javascript:void(0);" id="translateButton<?php echo $row1['comment_id'];?>"  ><?php echo  $lang['Translate'];?></a></span>
...
var lan1="<?php echo $_SESSION['lang'];?>";
var id="<?php echo $reply_res['reply_id'];?>";
var text="<?php echo $reply_res['content'];?>";
...
//alert(src);
    ...
    $.ajax({
        ...
        //alert(status);
			...
			translate_back1213<?php echo $w;?>(idbd,g_token,src,type,lan);
...
function translate_back1213<?php echo $w;?>(idbd,g_token,src1,type1,lan)
	{
		//alert("trnslate");
	 var language=lan;
...
$reply_like_query1 = mysqli_query($con, "SELECT m.active,m.username,m.member_id 
...
<span style="top:2px; margin-left:3px;" > 
<span class="mySpan_dot_class"> · </span>
<a class="replytranslateButton" href="javascript:void(0);" id="replytranslateButton<?php echo $reply_res['reply_id'];?>"  ><?php echo $lang['Translate']; ?></a></span>
...
<div tabindex="1" id="replytranslatemenu<?php echo $reply_res['reply_id'];?>" class="replytranslatemenu" style="display:none; position:absolute;margin-left: 34%; "> <select id="replylangs<?php echo $reply_res['reply_id'];?>" class="postlangs" onchange="selectOption(this.value, <?php echo $reply_res['reply_id'];?>,3,'<?php 
if(isset($_SESSION['lang']))
{
echo $_SESSION['lang'];
}
...
$repl_r_memb_id=$reply_r_res['member_id'];
	...
	$block_count = mysqli_num_rows($block);
if($block_count==0 && $reply_r_res['member_id'])
{
...
var lan1="<?php echo $_SESSION['lang'];?>";
var id="<?php echo $reply_r_res['id'];?>";
var text="<?php echo $reply_r_res['content'];?>";
...
$last_id = $row["messages_id"];
		 ...
		 // Create session for check and avoid dupilicate records
...
<script>
$("#hidden_ajax_lastid").val(<?php echo $lastid;?>);
$(document).ready(function () { 
	$(".see_more_link").click(function(e) {
    e.preventDefault();
	var id = $(this).attr("id");
	var sid = id.split("id");
    var New_ID = sid[1];	
		$(".id"+New_ID).addClass("text_exposed");
