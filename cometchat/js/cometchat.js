...
(function($){
    $.cometchat = $.cometchat||function(){
            ...
            if(!ccvariable.windowFocus){
                jqcc.each( ccvariable.openChatboxId, function( key, value ) {
                    if(typeof ccvariable.lastmessagereadstatus[value] != 'undefined' && ccvariable.lastmessagereadstatus[value] == 0){
                        ...
                        lastmessagereadid = jqcc[calleeAPI].lastreadmessageid(value);
                        var message = {"id": messageid, "from": value, "self": 0, "lastmessagereadid": lastmessagereadid};
                        if(ccvariable.currentStatus != 'invisible' && isTapatalk == 0){
                        ...
                        if(data != null && typeof(data) != 'undefined'){
                            ...
                            if(typeof (jqcc[calleeAPI].addMessages)!=='undefined'){
                                if(broadcastflag){
                                    ...
                                    jqcc[calleeAPI].addMessages([{"from": id, "message": data.m, "id": data.id, "broadcast": 0, "direction": data.direction}]);
                                }
                            }
        ...
        arguments.callee.processcontrolmessage = function(incoming){
            ...
            }else if((incoming.message).indexOf('has successfully sent a file')!=-1 || (incoming.message).indexOf('has sent you a file')!=-1){
                if(ccvariable.callbackfn=="desktop"){
            ...
            }else if((incoming.message).indexOf('has successfully sent a handwritten message')!=-1 || (incoming.message).indexOf('has sent you a handwritten message')!=-1){
        ...
        arguments.callee.closeCRPopout = function(params){
            if($.cookie(settings.cookiePrefix+'crstate')!=null){
                ...
                var crstate_val_new = crstate_val[0] +'|'+ crstate_val[1] +'|'+ params.popoutmode;
        ...
        arguments.callee.sendReceipt = function(incoming, receipt){
            ...
            var lastreadmessage;
            if(typeof incoming.lastmessagereadid == "undefined"){
                lastreadmessage = 0;
            } else {
                lastreadmessage = incoming.lastmessagereadid;
            }
            ...
            if(settings.cometserviceEnabled == 1 && settings.messagereceiptEnabled == 1 && typeof receipt != "undefined" && incoming.id != undefined){
                ...
                var controlmessage = 'CC^CONTROL_{"type":"core","name":"textchat","method":"'+receipt+'","params":{"fromid":'+ccvariable.userid+',"message":'+messageid+',"lastreadmessageid":'+lastreadmessage+'}}';
