		...
		return {
			...
			viewMedia: function (params) {
				...
				if(params.mediatype == 1){
					mediaContentData = '<img class="cometchat_filetransfer_data cometchat_filetransfer_image" md5fileName="'+params.md5file+'" fileName="'+params.file+'" src= "'+baseUrl+'plugins/filetransfer/uploads/'+params.md5file+'">';
				} else if(params.mediatype == 2){
					mediaContentData = '<video class="cometchat_filetransfer_data" width="360" height="260" md5fileName="'+params.md5file+'" fileName="'+params.file+'" controls autoplay><source src="'+baseUrl+'plugins/filetransfer/uploads/'+params.md5file+'" ></video>';
				} else if(params.mediatype == 3){
					mediaContentData = '<audio class="cometchat_filetransfer_data" md5fileName="'+params.md5file+'" fileName="'+params.file+'" controls><source src="'+baseUrl+'plugins/filetransfer/uploads/'+params.md5file+'"></audio>';
				}
...
jqcc(function(){
	...
	jqcc('.mediamessage').live('click',function(e){
		...
		if(jqcc.cometchat.getCcvariable().callbackfn!='undefined' && jqcc.cometchat.getCcvariable().callbackfn=='desktop' && mediatype!=1){
			var file = jqcc('.cometchat_filetransfer_data').attr('fileName');
			var md5file = jqcc('.cometchat_filetransfer_data').attr('md5fileName');
			location.href = baseUrl+"plugins/filetransfer/download.php?file="+md5file+"&unencryptedfilename="+file+"";
		}else{
