    ...
    $.ccsynergy = (function(){
        ...
        return {
            ...
            initialize: function(){
                        ...
                        if(trayicon.hasOwnProperty(x)){
                            if(x!='chatrooms'){
                                ...
                                if(jqcc.cometchat.getCcvariable().callbackfn=='desktop'){
                                    if(x != 'home' && x != 'scrolltotop' && x != 'themechanger' && x != 'share' && x !='translate2'){
                ...
                }else if(jqcc.cometchat.getThemeVariable('mobileDevice') && jqcc.cometchat.getBaseData()!=0 && jqcc.cometchat.getBaseData()!='null'){
                    var mobileOverlay = '';
                    if('<?php echo $p_; ?>'<3){
                        mobileOverlay = '<div class="cometchat_mobile_overlay"><p>'+language[94]+'</p></div>';
                    } else {
                        if(window.top==window.self){
                            window.location.href = 'extensions/mobilewebapp/index.php?cookiePrefix='+cookiePrefix+'&basedata='+jqcc.cometchat.getBaseData()+'&ccmobileauth='+jqcc.cometchat.getThemeVariable('ccmobileauth');
                        } else {
                            mobileOverlay = '<div class="cometchat_mobile_overlay"><p>'+language[94]+'<a target = "_blank" href = "'+jqcc.cometchat.getBaseUrl()+'extensions/mobilewebapp/index.php?cookiePrefix='+cookiePrefix+'&basedata='+jqcc.cometchat.getBaseData()+'&ccmobileauth='+jqcc.cometchat.getThemeVariable('ccmobileauth')+'" style="color:#1C97D0">Click Here</a> to access mobile compatible chat.</p></div>';
                        }
                    }
                ...
                $('.cometchat_trayiconimage').click(function(event){
                    ...
                    if(window.top == window.self || jqcc.cometchat.getCcvariable().callbackfn=='desktop') {
                        jqcc.cometchat.lightbox(moduleName,'',1);
                    } else {
            ...
            loggedOut: function(){
                ...
                if(settings.ccauth.enabled=="1" && !jqcc.cometchat.getThemeVariable('mobileDevice')){
                ...
                }else if(settings.ccauth.enabled=="1" && jqcc.cometchat.getThemeVariable('mobileDevice')){
            ...
            deliveredMessageNotify: function(item){
                ...
                } else if(typeof item['fromid'] != 'undefined'){
            ...
            readMessageNotify: function(item){
                ...
                lastreadmessages[item['fromid']]= parseInt(item['lastreadmessageid']);
                if(lastreadmessages[item['fromid']] != 0){
                    jqcc.cometchat.updateToStorage('lastreadmessages',lastreadmessages);
                }
            ...
            deliveredReadMessageNotify: function(item){
                var lastreadmessages={};
                lastreadmessages[item['fromid']]= parseInt(item['message']);
                if(lastreadmessages[item['fromid']] != 0){
                    jqcc.cometchat.updateToStorage('lastreadmessages',lastreadmessages);
                }
                if($("#cometchat_message_"+item['message']).length == 0){
            ...
            createChatboxData: function(id, name, status, message, avatar, link, isdevice, silent, tryOldMessages){
                ...
                if(typeof(cometchat_user_popup)=='undefined' || cometchat_user_popup.length<1){
                    ...
                    $("<div/>").attr("id", "cometchat_user_"+id+"_popup").addClass("cometchat_userchatbox").addClass("cometchat_tabpopup").css('display', 'none').html('<div class="cometchat_tabsubtitle"><div class="cometchat_chatboxMenuOptions">'+avchathtml+pluginshtml+'</div><div class="cometchat_chatboxLeftDetails">'+avatarsrc+'<div class="cometchat_chatboxDisplayDetails"><div class="cometchat_userdisplayname" title="'+longname+'">'+startlink+longname+endlink+'<span id="cometchat_typing_'+id+'" class="cometchat_typing"></span></div><div class="cometchat_userdisplaystatus" title="'+message+'">'+message+'</div></div></div><div class="cometchat_user_closebox" title="Close Chat Box">X</div></div>'+prepend+'<div class="cometchat_tabcontent"><div class="cometchat_tabcontenttext" id="cometchat_tabcontenttext_'+id+'"><div class="cometchat_message_container"></div></div><div class="cometchat_tabinputcontainer">'+filetransferhtml+'<div class="cometchat_tabcontentsubmit cometchat_sendicon" title="Send"></div><div class="cometchat_tabcontentinput">'+smilieshtml+'<div style="margin-right:28px;"><textarea class="cometchat_textarea"></textarea></div></div></div></div>').appendTo($("#cometchat_righttab"));
                        ...
                        if(jqcc.isEmptyObject(chatBoxesOrder)&&$.cookie(settings.cookiePrefix+'chatroom')!=null&&$.cookie(settings.cookiePrefix+'chatroom')!='' && hasChatroom=='1'){
                            ...
                            if(jqcc.cometchat.getThemeVariable('currentStatus')!='invisible'){
                                ...
                                if(typeof $("#cometchat_user_"+nextChatBox+"_popup .cometchat_message_container").find('.cometchat_chatboxmessage ').last().attr('id') != 'undefined'){
                                    messageid = $("#cometchat_user_"+nextChatBox+"_popup .cometchat_message_container").find('.cometchat_chatboxmessage ').last().attr('id').split('_')[2];
                                }
            ...
            addMessages: function(item){
                var lastreadmessage = jqcc.cometchat.getFromStorage('lastreadmessages');
                $.each(item, function(i, incoming){
                    ...
                    if(jqcc.cometchat.getThemeArray('buddylistName', incoming.from)==null||jqcc.cometchat.getThemeArray('buddylistName', incoming.from)==''){
                        ...
                        if($("#cometchat_message_"+incoming.id).length>0){
                            ...
                            if(settings.cometserviceEnabled == 1 && settings.messagereceiptEnabled == 1 && incoming.id <= alreadyreceivedunreadmessages[incoming.from] && incoming.self == 1 && incoming.old == 1 && incoming.direction == 0){
                                $("#cometchat_chatboxseen_"+incoming.id).addClass('cometchat_deliverednotification');
                            }
                            if(settings.cometserviceEnabled == 1 && settings.messagereceiptEnabled == 1 && incoming.id <= lastreadmessage[incoming.from] && incoming.self == 1){
                                $("#cometchat_chatboxseen_"+incoming.id).addClass('cometchat_readnotification');
                            }
                            var nowTime = new Date();
                            ...
                            if(typeof($.cookie(settings.cookiePrefix+'chatroom'))!=='undefined' && hasChatroom==1 && $.cookie(settings.cookiePrefix+'chatroom')!=null && $.cookie(settings.cookiePrefix+'chatroom')!=''){
                                if(cc_states[5]=='chatrooms'&&jqcc.cometchat.getThemeVariable('trayOpen')!=cc_states[5]){
                                    ...
                                    if(typeof(cc_chatroomdetails[0])!='undefined' && typeof(cc_chatroomdetails[1])!='undefined' && typeof(cc_chatroomdetails[2])!='undefined'&&jqcc.cometchat.getThemeVariable('chatroomOpen')!=cc_chatroomdetails[0]){
                                        jqcc.cometchat.silentroom(cc_chatroomdetails[0], cc_chatroomdetails[1], cc_chatroomdetails[2]);
                ...
                if(typeof (jqcc[settings.theme].addMessages)!=='undefined'&&data.hasOwnProperty('messages')){
                    ...
                    if(jqcc.cometchat.getThemeVariable('currentStatus')!='invisible'){
                        ...
                        if(typeof $("#cometchat_user_"+id+"_popup").find("div.cometchat_chatboxmessage:last-child").attr('id') != 'undefined'){
                            messageid = $("#cometchat_user_"+id+"_popup").find("div.cometchat_chatboxmessage:last-child").attr('id').split('_')[2];
                        }
                ...
                if(jqcc.cometchat.getThemeVariable('loggedout')==0 && jqcc.cometchat.getUserID() != id){
                    ...
                    if(jqcc.cometchat.getThemeVariable('currentStatus')!='invisible'){
                       ...
                       if(typeof $("#cometchat_user_"+id+"_popup .cometchat_message_container").find('.cometchat_chatboxmessage ').last().attr('id') != 'undefined'){
                            messageid = $("#cometchat_user_"+id+"_popup .cometchat_message_container").find('.cometchat_chatboxmessage ').last().attr('id').split('_')[2];
                        }
                        var message = {"id": messageid, "from": id, "self": 0};
                        jqcc.cometchat.sendReceipt(message, 'readMessageNotify');
                    }
