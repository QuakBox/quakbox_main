            ...
            initialize: function(){
                ...
                $("#cometchat").find("div.cometchat_popchatrooms").click(function(){
                    ...
                    if($.cookie(settings.cookiePrefix+'crstate')!=null) {
                        var crstate_val = $.cookie(settings.cookiePrefix+'crstate').split('|');
                        var crstate_val_new = crstate_val[0] +'|'+ crstate_val[1] +'|'+ popoutmode;
                        ...
                        if(jqcc.cometchat.getThemeVariable('trayOpen')==''){
                            ...
                            if(id=='chatrooms'){
                                if($.cookie(settings.cookiePrefix+'crstate')!=null){
                                    var msgcount = 0;
                                    var popoutmode = 0;
                                    var crstate_val_new = msgcount +'|'+ parseInt($.cookie(settings.cookiePrefix+'crstate').split('|')[1]) +'|'+ popoutmode;
                                    $.cookie(settings.cookiePrefix+'crstate', crstate_val_new, {path: "/"});
                                }
                            }
                ...
                if(typeof(jqcc.cookie(settings.cookiePrefix+'chatroom')) != 'undefined' && typeof(trayicon['chatrooms']) != 'undefined' && jqcc.cookie(settings.cookiePrefix+'chatroom') != null) {
                    var chatroom_data = a2b(jqcc.cookie(settings.cookiePrefix+'chatroom')).split(":");
                    chatroom_data[0] = chatroom_data[0]||0;
                    chatroom_data[1] = chatroom_data[1]||"";
                    chatroom_data[2] = chatroom_data[2]||"";
            ...
            buddyList: function(item){
                ...
                $.each(item, function(i, buddy){
                    ...
                    if((buddy.s != 'offline' && settings.hideOffline == 1) || settings.hideOffline == 0){
                        ...
                        buddylisttempavatar += group+'<div id="cometchat_userlist_'+buddy.id+'" class="cometchat_userlist"><span class="cometchat_userscontentavatar"><img class="cometchat_userscontentavatarimage" original="'+buddy.a+'"><span class="cometchat_userscontentdot cometchat_'+usercontentstatus+'">'+icon+'</span></span><div class="cometchat_chatboxDisplayDetails"><div class="cometchat_userdisplayname">'+longname+'</div><span id="cometchat_buddylist_typing_'+buddy.id+'" class="cometchat_buddylist_typing"></span><div class="cometchat_userdisplaystatus">'+buddy.m+'</div></div></div>';
                    }
                    var message = '';
