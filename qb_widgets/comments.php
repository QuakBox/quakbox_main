		...
		if($countOfAllComments>0){
			foreach($viewAllComments as $valueCurrentComment) {	
				...
				if($commentMemberProfileImage){			
						$commentMemberProfileImage=SITE_URL.'/'.$commentMemberProfileImage."?".time();	
				}
				else{
				...
				$userLink='';
				$commentUser.='<div class="comment_'.$encryptedcommentId.'" style="padding:5px;margin-bottom: 3px;background:#EDEDED;">';
				...
				$commentUser.='</div>';
				$commentUser.='<div>';
                    $srt1 = substr($commentContent, 0, 400);
                    $srt2 = substr($commentContent, 401);
					...
					$text=$srt1;
						...
						$commentUser.= preg_replace($reg_exUrl, "<a href='{$url[0]}'> {$url[0]} </a> ", $text);
                    ...
                    if(!empty($srt2)){
                        $commentUser.='<span>...</span> <a class="btnreadmore" href="javascript: void(0)">Read more</a> <div class="more-content" style="display:none;">'.$srt2.'</div>';
                    }
