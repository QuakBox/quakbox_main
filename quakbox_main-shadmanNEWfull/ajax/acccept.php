...
if ($item == 'ads') {
    ...
    if ($post_ID_From_Post != '') {
        ...
        if ($post_action == 'like') {
        ...
        else if ($post_action == 'delComment') {
            $postID = $QbSecurity->QB_AlphaID($post_ID_From_Post, true);
            include($_SERVER['DOCUMENT_ROOT'] . '/qb_classes/qb_post.php');
            $obj = new posts();
            $affected = $obj->delete_comment($postID);
        ...
        } else if ($post_action == 'delReply') {
            $postID = $QbSecurity->QB_AlphaID($post_ID_From_Post, true);
            include($_SERVER['DOCUMENT_ROOT'] . '/qb_classes/qb_post.php');
            $obj = new posts();
            $affected = $obj->delete_comment_reply($postID);
        ...
        } else if ($post_action == 'delReplyReply') {
            $postID = $post_ID_From_Post;
            include($_SERVER['DOCUMENT_ROOT'] . '/qb_classes/qb_post.php');
            $obj = new posts();
            $affected = $obj->delete_comment_reply_reply($postID);
            ...
            $commentContent = nl2br(strip_tags($_POST['content']));
            $postID = $QbSecurity->QB_AlphaID($post_ID_From_Post, true);
            include($_SERVER['DOCUMENT_ROOT'] . '/qb_classes/qb_post.php');
            ...
            $replyContent = nl2br(strip_tags($_POST['content']));
            $commentID = $QbSecurity->QB_AlphaID($post_ID_From_Post, true);
            include($_SERVER['DOCUMENT_ROOT'] . '/qb_classes/qb_post.php');
        ...
        //$urlPattern = '/((https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/i';
        ...
        $wallID = $objLookupClass->getLookupKey('Wall Type', 'Country');;
       ...
       // $wallID = $QbSecurity->QB_AlphaID($wall, true);
        ...
        if(count($matches) > 0) {
            include($_SERVER['DOCUMENT_ROOT'].'/fetching/fetch_url_data.php');
            ...
            $type = isset($ogTags['og:type']) ? $ogTags['og:type'] : '';

            if( (trim($linkType) == 'video') && (isset($tags['twitter:player'])) ) {
                $content = '<iframe src="'.$tags['twitter:player'].'" frameborder="0" allowfullscreen></iframe>';
            }
        }*/
      ...
      //  $affected = $obj->insert_post_status($loggedin_member_id_for_ajax, $content, $wall, $ip, $wallItem);
        $affected = $obj->insert_post_status($loggedin_member_id_for_ajax, $content, $wallID, $ip, $wallItem);
        ...
        if ($wall != 'group') {
           // $affected = $obj->insert_post_image($loggedin_member_id_for_ajax, $content, $wall, $ip, 0, $upload_image, $wallItem);
            $affected = $obj->insert_post_image($loggedin_member_id_for_ajax, $content, $wallID, $ip, 0, $upload_image, $wallItem);
        } else {
            //$encGroupID=$_POST['d'];
        ...
        if ($wall != 'group') {
           // $affected = $obj->insert_post_video($loggedin_member_id_for_ajax, $title, $description, $wall, $ip, 0, $videoname, $wallItem);
            $affected = $obj->insert_post_video($loggedin_member_id_for_ajax, $title, $description, $wallID, $ip, 0, $videoname, $wallItem);
        } else {
            //$encGroupID=$_POST['d'];
            //include($_SERVER['DOCUMENT_ROOT'].'/qb_classes/qb_groups.php');
            //$objGroup = new groups();
