                                      <?php
/**
   * democountry.php helps to fetch old posts in countrywall
   * 
   * @author     Vishnu
   * Created date  02/20/2015 04:40:05
   * Updated date  03/23/2015 05:00:05
...
ini_set('max_execution_time', 300); 	
	...
	require_once('common/qb_security.php');
		require_once('common/qb_validation.php');
?>
<script>
$('.translateButton<?php echo $lastid;?>').click(function (event) {
        var ID = $(this).attr('id');
    ...
    $('.posttranslateButton<?php echo $lastid;?>').click(function (event) {
        var ID = $(this).attr('id');
	...
	$('.replytranslateButton<?php echo $lastid;?>').click(function (event) {
	    var ID = $(this).attr('id');
...
ini_set('max_execution_time', 300); 	
	...
	$uname=$res["username"];
	$lastid = $QbSecurity->qbClean($_POST["lastid"], $con); // save the posted value in a variable
...
$query = "SELECT msg.messages_id, msg.msg_album_id, msg.messages,msg.date_created, 
			msg.country_flag, msg.type, m.member_id, m.active, m.profImage,msg.video_id,msg.share,
			msg.share_by, msg.wall_privacy,
			m.username, u.upload_data_id,
...
$post_like_sql1 = mysqli_query($con, "SELECT m.username,m.active,m.member_id FROM bleh b, members m WHERE m.active != 1 AND m.member_id=b.member_id AND b.remarks='".$row['messages_id']."' AND b.member_id='".$member_id."'");
$post_like_count1 = mysqli_num_rows($post_like_sql1);

if($post_like_count1==1)
{
$post_like_sql2 = mysqli_query($con, "SELECT m.username,m.active,m.member_id FROM bleh b, members m WHERE m.active != 1 AND m.member_id=b.member_id AND b.remarks='".$row['messages_id']."' AND b.member_id!='".$member_id."' LIMIT 2");
...
<span class="mySpan_dot_class"> · </span>
 <a class="translateButton<?php echo $lastid;?> show_cmt_linkClr" href="javascript:void(0);" id="translateButton<?php echo $row1['comment_id'];?>"  ><?php echo  $lang['Translate'];?></a></span>
...
$reply_like_query2 = mysqli_query($con, "SELECT m.username,m.active,m.member_id 
								  ...
								  WHERE m.active != 1 AND m.member_id=c.member_id 
								  AND c.reply_id='".$reply_res['reply_id']."' 
								  AND c.member_id!='".$member_id."' LIMIT 2");
...
<?php if($row1['type']==1)
{ ?>
<span style="top:2px; margin-left:3px;" > <a class="replytranslateButton<?php echo $lastid;?> show_cmt_linkClr" href="javascript:void(0);" id="replytranslateButton<?php echo $reply_res['reply_id'];?>"  ><?php echo $lang['Translate']; ?></a></span>
...
<div class="reply-reply-text">
 <?php 
if($member_id == $reply_r_res['member_id'])
{
?>
...
<span class="mySpan_dot_class"> · </span>
<a class="posttranslateButton<?php echo $lastid;?> show_cmt_linkClr" href="javascript:void(0);" id="posttranslateButton<?php echo $row['messages_id'];?>"  ><?php echo $lang['Translate'];?></a>
</span>
<?php } } ?>
</div>
