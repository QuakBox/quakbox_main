...
?></div><?php
$user= mysqli_query($con, "select * from members where member_id = '".$_SESSION['SESS_MEMBER_ID']."'");
	$user_name=mysqli_fetch_array($user);
	$u_name=$user_name["username"];
?>
<a href="<?php echo $base_url;?>albums.php?back_page=<?php echo $base_url.'i/'.$u_name;?>&member_id=<?php echo $row['member_id']; ?>&album_id=<?php echo $row['msg_album_id']; ?>&image_id=<?php echo $row['upload_data_id'];?>" >
...
if(isset($_SESSION['lang'])&&($_SESSION['lang']<>"en"))
	{	
		...
		$sql = mysqli_query($con, "select * from reply_reply1 where msg_id='".$reply_r_res['id']."' and tr_id='".$_SESSION['lang']."'")or die(mysqli_error());
$r_count = mysqli_num_rows($sql);
if($r_count>0)
{
