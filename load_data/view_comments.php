...
$totals = (int)$_REQUEST['totals'];
	$comment1234 = mysqli_query($con, "SELECT * FROM postcomment c LEFT JOIN members m ON m.member_id = c.post_member_id where msg_id = ".$_REQUEST['postId']." order by date_created desc limit 4, ".$totals);
	$comment_num_row = mysqli_num_rows(@$comment1234);
...
for($loop=$loop_count-1;$loop>=0;$loop--)
{
  $result_arr[$loop];
	...
	//$comments = mysqli_query("SELECT * FROM postcomment c LEFT JOIN members m ON m.member_id = c.post_member_id where msg_id = ".$_REQUEST['postId']." //order by date_created desc limit 4, ".$_REQUEST['totals']);
$comments = mysqli_query($con, "SELECT * FROM postcomment c LEFT JOIN members m ON m.member_id = c.post_member_id where msg_id = ".$_REQUEST['postId']." and c.comment_id='".$result_arr[$loop]."'");
...
$rows = mysqli_fetch_array($comments);
...
<a href="<?php echo $base_url.$rows['username'];?>"><img src="<?php echo $base_url. $rows['profImage']; ?>" class='small_face'/></a>
...
if(isset($_SESSION['lang']))
	{	
		...
		$sql = mysqli_query($con, "select * from postcomment1 where msg_id='".$rows['comment_id']."' and tr_id='".$_SESSION['lang']."'")or die(mysqli_error($con));
...
$sql = mysqli_query($con, "SELECT * FROM comment_like WHERE comment_id='". $rows['comment_id'] ."'");
...
$cdquery = "SELECT * FROM comment_dislike WHERE comment_id='". $rows['comment_id'] ."'";
...
$comment_like = mysqli_query($con, "select * from comment_like where comment_id = '".$rows['comment_id']."' and member_id = '".$member_id."'");

if(mysqli_num_rows($comment_like) > 0)

{

	echo '<a href="javascript: void(0)" class="comment_like" id="comment_like'.$rows['comment_id'].'" msg_id = '.$rows['msg_id'].' title="'.$lang['Unlike'].'" rel="Unlike">'.$lang['Unlike'].'</a>';
...
{ 
	...
	echo '<a href="javascript: void(0)" class="comment_like" id="comment_like'.$rows['comment_id'].'" msg_id = '.$rows['msg_id'].' title="'.$lang['Like'].'" rel="Like">'.$lang['Like'].'</a>';
...
$cdquery1 = "SELECT * FROM comment_dislike WHERE comment_id='". $rows['comment_id'] ."' and member_id = '".$member_id."'";
...
echo '<a href="javascript: void(0)" class="comment_dislike" id="comment_dislike'.$rows['comment_id'].'" title="'.$lang['Undislike'].'" rel="undislike">'.$lang['Undislike'].'</a>';
} else {
...
$query12  = mysqli_query($con, "SELECT * FROM comment_reply WHERE comment_id=" . $rows['comment_id'] . " ORDER BY reply_id DESC");

$records1 = mysqli_num_rows($query12);

$p = mysqli_query($con, "SELECT * FROM comment_reply WHERE comment_id=" . $rows['comment_id'] . " ORDER BY reply_id DESC limit 2,$records1");
...
$reply_sql  = mysqli_query($con, "SELECT * FROM comment_reply c,members m WHERE c.member_id = m.member_id and comment_id=" . $rows['comment_id'] . " ORDER BY reply_id DESC limit 0,2");
...
echo '<a href="javascript: void(0)" class="reply_dislike" id="reply_dislike'.$reply_res['reply_id'].'" title="'.$lang['Undislike'].'" rel="undisLike">'.$lang['Undislike'].'</a>';
...
<input type="submit" abcd="<?php echo $rows['member_id']; ?>"  title="<?php echo $rows['username']; ?>" value="<?php echo $lang["Reply"];?>"  id="<?php echo $rows['comment_id'];?>" class="reply_button"/>

<input type="button"  value=" <?php echo $lang["Cancel"];?> "  id="<?php echo $rows['msg_id'];?>" onclick="closereply('replybox<?php echo $rows['comment_id'];?>')" class="cancel"/>
