...
$dislikequery = "SELECT * FROM comment_dislike WHERE member_id='$uid' and comment_id='$comment_id'";
$dislikesql = mysqli_query($con, $dislikequery);

//like data fetch using following query
$likequery = "SELECT * FROM comment_like WHERE member_id='$uid' and comment_id='$comment_id'";
$likesql = mysqli_query($con, $likequery);

$q=mysqli_query($con, "SELECT * FROM comment_like WHERE member_id='$uid' and comment_id='$comment_id' ");

if(mysqli_num_rows($q)==0)
{
$query=mysqli_query($con, "INSERT INTO comment_like (msg_id,comment_id,member_id) VALUES('$msg_id','$comment_id','$uid')");

$g = mysqli_query($con, "SELECT like_id FROM comment_like WHERE comment_id = '$comment_id'");
$like_count = mysqli_num_rows($g);

if(mysqli_num_rows($dislikesql) > 0) {
		$query=mysqli_query($con, "DELETE FROM comment_dislike WHERE comment_id='$comment_id' and member_id='$uid'");			
	$h = mysqli_query($con, "SELECT dislike_comment_id FROM comment_dislike WHERE comment_id = '$comment_id'");
