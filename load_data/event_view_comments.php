<?php ob_start();
	...
	$comments = mysqli_query($con, "SELECT * FROM event_wall_comment c LEFT JOIN members m ON m.member_id = c.post_member_id where msg_id = ".$_REQUEST['postId']." order by date_created desc limit 4, ".$totals);
...

	{
		...
		while ($row1 = mysqli_fetch_array($comments))
...
if(isset($_SESSION['lang'])&&($_SESSION['lang']<>"en"))
	{	
		...
		$sql = mysqli_query($con, "select * from postcomment1 where msg_id='".$row1['comment_id']."' and tr_id='".$_SESSION['lang']."'")or die(mysqli_error());
