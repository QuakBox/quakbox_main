...
$g = mysqli_query($con, "SELECT bleh_id FROM groups_wall_like WHERE remarks = '$msg_id'");
...
if(mysqli_num_rows($dislikesql) > 0) {
	...
	$h = mysqli_query($con, "SELECT dislike_id FROM groups_wall_dislike WHERE msg_id = '$msg_id'");
	$dislike_count = mysqli_num_rows($h);	
}

$msgsql = mysqli_query($con, "SELECT m.member_id,m.email_id,msg.type,msg.messages FROM groups_wall msg LEFT JOIN members m ON m.member_id = msg.member_id 
...
$to = $msgres['email_id'];
$subject = "".$member_res['username']." likes your ".$subject_msg."";
$message = "
<html>
<head>
...
$g = mysqli_query($con, "SELECT bleh_id FROM groups_wall_like WHERE remarks = '$msg_id'");
$like_count = mysqli_num_rows($g);
$dislikequery = "SELECT * FROM groups_wall_dislike WHERE msg_id='$msg_id'";
$dislikesql = mysqli_query($con, $dislikequery);
