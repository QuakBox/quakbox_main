...
$commandformp4 = "".$ffmpeg." -i ".$pathforinputexec." -vf 'movie=".$watermark_path." [watermark]; [in][watermark] overlay=10:main_h-overlay_h-10 [out]' -y  -c:a aac -strict -2 -b:a 128k -c:v libx264 -profile:v baseline ".$pathforoutputexecMP4." 1> ".$PathForOutputLog." 2>&1";
...
/*	
$commandforOGG = "".$ffmpeg." -i ".$pathforoutputexecMP4." -acodec libvorbis -vcodec libtheora -f ogv ".$pathforoutputexecOGG." 1> 
".$PathForOutputLog." 2>&1";

$commandforOGG = "".$ffmpeg." -i ".$pathforinputexec." -vf 'movie=".$watermark_path." [watermark]; [in][watermark] overlay=10:main_h-overlay_h-10 [out]' -codec:v libtheora -qscale:v 5 -codec:a libvorbis -qscale:a 4 ".$pathforoutputexecOGG." 1> ".$PathForOutputLog." 2>&1";	

	shell_exec($commandforOGG);

	$commandforWEBM = "".$ffmpeg." -y -i ".$pathforinputexec." -vf 'movie=".$watermark_path." [watermark]; [in][watermark] overlay=10:main_h-overlay_h-10 [out]' -c:v libvpx -minrate 1M -maxrate 1M -b:v 1M -c:a libvorbis ".$pathforoutputexecWEBM." 1> ".$PathForOutputLog." 2>&1";*/

	/*$commandforWEBM = "".$ffmpeg." -i ".$pathforinputexec." -vf 'movie=".$watermark_path." [watermark]; [in][watermark] 
	overlay=10:main_h-overlay_h-10 [out]'  -c:a libvorbis -ac 1 -b:a 96k -ar 48000 -b:v 1100k -maxrate 1100k -bufsize 1835k -vcodec
	 libvpx -cpu-used -5 -deadline realtime  ".$pathforoutputexecWEBM." 1> ".$PathForOutputLog." 2>&1";

	shell_exec($commandforWEBM);*/
 ...
 $location =$upload_dir. $video_name;
	...
	$command = "".$ffmpeg." -i ".$pathforoutputexecMP4." -vf 'select=gt(scene\,0.5)' -frames:v 5 -vsync vfr -y ".$pathforthumbimage."";
	...
	exec("".$ffmpeg." -i ".$pathforoutputexecMP4." -deinterlace -an -ss ".$duration1." -f mjpeg -t 1 -r 1 -y ".$pathforthumbimage1."". ' 2>&1', $output1);

	exec("".$ffmpeg." -i ".$pathforoutputexecMP4." -deinterlace -an -ss ".$duration2." -f mjpeg -t 1 -r 1 -y ".$pathforthumbimage2."". ' 2>&1', $output2);

	exec("".$ffmpeg." -i ".$pathforoutputexecMP4." -deinterlace -an -ss ".$duration3." -f mjpeg -t 1 -r 1 -y ".$pathforthumbimage3."". ' 2>&1', $output3);

	exec("".$ffmpeg." -i ".$pathforoutputexecMP4." -deinterlace -an -ss ".$duration3." -f mjpeg -t 1 -r 1 -y ".$pathforthumbimage4."". ' 2>&1', $output4);

	exec("".$ffmpeg." -i ".$pathforoutputexecMP4." -deinterlace -an -ss ".$duration2." -f mjpeg -t 1 -r 1 -y ".$pathforthumbimage5."". ' 2>&1', $output5);
	...
	unlink($RemoveSpaceForBug);	
		  echo "Video Has been Processed Successfully!!";
