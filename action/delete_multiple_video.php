...
foreach($_POST['check'] as $value){
    ...
    $sqlforthumburl = mysqli_query($con, "select * from videos where video_id = '$id' AND user_id = '$session_id'");
    ...
    $msql = mysqli_query($con, "select * from videos where video_id = '$id'");
    $mres = mysqli_fetch_array($msql);
    $messages_id = $mres['msg_id'];
...
//remove videos thumb file from server
    ...
    if(mysqli_num_rows($msql) > 0)
    {
        ...
        mysqli_query($con, "DELETE FROM reply_reply WHERE reply_id ='$reply_id'");
    }
