<?php 
/**
   * This page helps to comment on an event
   * 
   * @package    Event
   * @author     Vishnu NCN
...
$smembersql =  mysqli_query($con, "select * from members where member_id='".$member_id."'");

$smemberres = mysqli_fetch_array($smembersql);

mysqli_query($con, "INSERT INTO event_wall_comment (member_id,msg_id,content, date_created,type)
VALUES('$member_id','$msg_id','$comment','".strtotime(date("Y-m-d H:i:s"))."',1)");

$sql = mysqli_query($con, "select * FROM event_wall_comment p,members m  WHERE p.member_id=m.member_id and p.msg_id = '$msg_id' order by comment_id desc");
...
if(isset($_SESSION['lang'])&&($_SESSION['lang']<>"en"))
	{	
		...
		$sql = mysqli_query($con, "select * from postcomment1 where msg_id='".$res['comment_id']."' and tr_id='".$_SESSION['lang']."'")or die(mysqli_error($con));
...
$row_comment = mysqli_fetch_assoc($sql);
	...
	$sql1 =  mysqli_query($con, "select * from postcomment1 where msg_id='".$res['comment_id']."' and tr_id='".$_SESSION['lang']."'")or die(mysqli_error($con));
...
<div>
<?php
$sql = mysqli_query($con, "SELECT * FROM event_wall_comment_like WHERE comment_id='". $res['comment_id'] ."'");
...
if($comment_like_res1==1)
{?><span id="you<?php echo $res['comment_id'];?>"><a href="#"><?php echo $lang['You'];?> </a><?php if($comment_like_count>1)
echo ','; ?> </span><?php
}
...
<div>
<?php
$cdquery = "SELECT * FROM event_wall_comment_dislike WHERE comment_id='". $res['comment_id'] ."'";
...
<span style="top:2px;">
<?php
$comment_like = mysqli_query($con, "select * from  event_wall_comment_like where comment_id = '".$res['comment_id']."' and member_id = '".$member_id."'");
if(mysqli_num_rows($comment_like) > 0)
{
	echo '<a href="javascript: void(0)" class="comment_like show_cmt_linkClr" id="comment_like'.$res['comment_id'].'" msg_id = '.$row['messages_id'].' title="'.$lang['Unlike'].'" rel="Unlike">'.$lang['Unlike'].'</a>';
} 
else 
{ 
	echo '<a href="javascript: void(0)" class="comment_like show_cmt_linkClr" id="comment_like'.$res['comment_id'].'" msg_id = '.$row['messages_id'].' title="'.$lang['Like'].'" rel="Like">'.$lang['Like'].'</a>';
}
?>
...
<?php
$cdquery1 = "SELECT * FROM event_wall_comment_dislike WHERE comment_id='". $res['comment_id'] ."' and member_id = '".$member_id."'";
...
echo '<a href="javascript: void(0)" class="comment_dislike show_cmt_linkClr" id="comment_dislike'.$res['comment_id'].'" title="'.$lang['Undislike'].'" rel="disLike">'.$lang['Undislike'].'</a>';
}
?>
...
<span style="top:2px; margin-left:2px;" >
<span class="mySpan_dot_class"> · </span>
 <a class="translateButton show_cmt_linkClr" href="javascript:void(0);" id="translateButton<?php echo $res['comment_id'];?>"  ><?php echo  $lang['Translate'];?></a>
...
<!--View more reply-->
<?php
$query12  = mysqli_query($con, "SELECT * FROM event_wall_comment_reply WHERE comment_id=" . $res['comment_id'] . " ORDER BY reply_id DESC");
$records1 = mysqli_num_rows($query12);
$p = mysqli_query($con, "SELECT * FROM event_wall_comment_reply WHERE comment_id=" . $res['comment_id'] . " ORDER BY reply_id DESC limit 2,$records1");
$q = mysqli_num_rows($p);
if ($records1 > 2)
{
	$collapsed1 = true;?>
...
<?php
$reply_sql  = mysqli_query($con, "SELECT * FROM event_wall_comment_reply c,members m WHERE c.member_id = m.member_id and comment_id=" . $res['comment_id'] . " ORDER BY reply_id DESC limit 0,2");
...
<input type="button"  value=" <?php echo $lang["Cancel"];?> "  id="<?php echo $row['messages_id'];?>" onclick="closereply('replybox<?php echo $res['comment_id'];?>')" class="cancel"/>
</form>
</div>
</div>
<!--End replyupdate div	--> 
