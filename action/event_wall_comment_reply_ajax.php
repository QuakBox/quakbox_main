...
$reply = mysqli_real_escape_string($con, f($_POST['reply'],'escapeAll'));
...
mysqli_query($con, "INSERT INTO event_wall_comment_reply (member_id,comment_id,content, date_created)
VALUES('$member_id','$comment_id','$reply','".strtotime(date("Y-m-d H:i:s"))."')");

$sql = mysqli_query($con, "select * from event_wall_comment_reply a, members b where a.member_id = b.member_id AND comment_id = '$comment_id' order by reply_id desc");
$reply_res = mysqli_fetch_array($sql);
if ($reply_res)
{
	$com_id = $reply_res['reply_id'];	
	$comment = $reply_res['content'];
	$time = $reply_res['date_created'];
...
<?php 
 ...
 if($smemberres['member_id'] <> $reply_res['member_id'])
 {
	 echo '@'; 
	?> <a href="<?php echo $base_url.$smemberres['username'];?>"><b><?php echo $res['username']; ?> 
