...
while($row2=mysqli_fetch_array($st_fre))
{
	...
	if($fred_id=="")
	 ...
	 {
		 $fred_id=$fred_id.','.$f_id;
	 }
}
//echo $fred_id; 		
$separated_id=explode(',',$fred_id);
foreach($separated_id as $sepf_id)
{
	//echo $sepf_id;
}
...
while($row = mysqli_fetch_assoc($result))
{
	...
	//{
	 //echo $row['wall_privacy'];	
	///echo $_SESSION['SESS_MEMBER_ID'];
	 
	 
	 		
	 		
//code to show only for  member and public
		
//echo "member id=".$memb_id;		
	
	
	if((($member_id==$memb_id) && $row['wall_privacy']==3)||($row['wall_privacy']==1))
	{
?>
<script type="text/javascript"> 
$(document).ready(function(){$("#stexpand<?php echo $msg_id;?>").oembed("<?php echo  $orimessage; ?>",{maxWidth: 400, maxHeight: 300});});
</script>	
<div class="stbody" id="stbody<?php echo $row['messages_id'];?>" data-id="<?php echo $row['messages_id'];?>">
...
<span class="show-cmt">
<a href="javascript:void(0)" class="share_open" id="<?php echo $row['messages_id'];?>" title="Share">Share</a>
</span>
